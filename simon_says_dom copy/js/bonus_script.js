...
function validation (user_numbers){

    let bool = false
    for(let i = 0; i < user_numbers.length; i++){

        if (user_numbers[i].length > 1){
            bool = true
            return bool
        }
...
//Per l'evento click del bottone confirm...
confirm_button_El.addEventListener('click', (e) => {
    ...
    //Dichiaro la variabile di lavoro per la validazione dei valori in input
    const number_validation = validation(user_numbers)
    ...
    if(number_validation == true){
        ...
        message_El.classList.add('d-block')
        confirm_button_El.classList.add('d-none')
        message_El.innerText = 'ATTENZIONE, devi inserire dei valori validi'
    }
